...
if($ergebnis!==false) {
	//sofern die zurückgelieferten Daten nicht false waren (dh. der DB-Server keinen Fehler geliefert hat), wird der nachfolgende Code ausgeführt
	$daten = $ergebnis->fetch_object(); //fetch_object: liefert einen Datensatz in Form eines Objektes; fetch_assoc: liefert einen Datensatz in Form eines assoziativen Arrays; fetch_array: liefert einen Datensatz in Form eines gemischt assoziativen Arrays (sowohl indiziert, als auch assoziativ --> alle Einträge stehen doppelt drinnen)
	ta($daten);
